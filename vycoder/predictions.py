...
max_distance = 8
positions = [
    [[0] * 256 for _ in range(257)] for _ in range(max_distance)
]  # dimensions [max_distance, 257, 256]
...
def weighted_positions(distance_weight, alpha, beta):
    def f(lst):
        ...
        for i in reversed(range(max_distance)):
            if i < len(lst):
                out = [
                    distance_weight(i) * x + y
                    for x, y in zip(positions[i][lst[-i - 1]], out)
                ]
            elif i == 0:
                out = [
                    distance_weight(i) * x + y
                    for x, y in zip(positions[i][256], out)
                ]
        out = [
            x * (sum(frequencies) + 256 * alpha) + beta * (y + alpha)
            for x, y in zip(out, frequencies)
        ]
        return out
